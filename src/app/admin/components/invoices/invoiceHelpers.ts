...
export const LEGAL_DISCLAIMERS: Record<PaymentTermsType, string> = {
  full: "",
  deposit_balance:
    `DEPOSIT & PAYMENT TERMS: The deposit amount listed above is non-refundable and secures your scheduled service date. The remaining balance is due in full upon completion of work unless otherwise agreed in writing. Failure to remit the balance within 7 days of job completion will result in a $50.00 late fee plus interest of 1.5% per month on the unpaid amount. Jenkins Home & Property Solutions reserves the right to suspend or cancel services if payment is not received as agreed.`,
  deposit_installments:
    `INSTALLMENT PLAN TERMS: The deposit amount listed above is non-refundable and secures your scheduled service date. The remaining balance will be paid in the installments shown above according to the due dates listed. All installment payments are due on or before their scheduled dates. Late payments will incur a $50.00 late fee plus interest of 1.5% per month on the overdue amount. If any installment is more than 14 days past due, the full remaining balance becomes immediately due and payable. Jenkins Home & Property Solutions reserves the right to suspend ongoing services until the account is current.`,
};
