...
export default function AdminQuotes({ userId, backRef, onNavigate, onSwitchToInvoice, initialQuoteId, onInitialQuoteConsumed }: {
      ...
      const res = await fetch("/api/pdf/preview", {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            due_date: (quote as any).due_date,
            status: quote.status,
            ...
            discount_amount: (quote as any).discount_type === "percent" ? quote.subtotal * ((quote as any).discount_value || 0) / 100 : (quote as any).discount_type === "amount" ? (quote as any).discount_value : 0,
            discount_reason: (quote as any).discount_reason || null,
            discount_percent: (quote as any).discount_type === "percent" ? (quote as any).discount_value : null,
            total: quote.total,
            notes: quote.notes,
            service_address: (quote as any).service_address || null,
            scope_summary: (quote as any).scope_summary || null,
            ai_project_notes: (quote as any).ai_project_notes || null,
            start_date: (quote as any).start_date || null,
            completion_date: (quote as any).completion_date || null,
            exclusions: (quote as any).exclusions || null,
            warranty: (quote as any).warranty || null,
            closing_statement: (quote as any).closing_statement || null,
            terms_conditions: (quote as any).terms_conditions || null,
          },
        }),
      });
      if (res.ok) {
      ...
      const res = await fetch("/api/pdf/preview", {
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            discount_percent: form.discount_type === "percent" ? form.discount_value : null,
            total,
            notes: form.notes,
  ...
  const handleSaveQuote = async (asDraft = true) => {
    ...
    const payload: Record<string, unknown> = {
      ...
      discount_type: form.discount_type === "target" ? "amount" : form.discount_type,
  ...
  const startEditQuote = (quote: Quote) => {
    ...
    setForm({
      ...
      discount_reason: (quote as any).discount_reason || "",
      notes: quote.notes || "",
