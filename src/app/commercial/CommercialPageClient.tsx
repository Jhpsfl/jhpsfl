...
const DEFAULT_EMAIL = "info@jhpsfl.com";
...
interface SanityData {
  ...
  solutions?: { icon: string; title: string; description: string; image?: { asset?: { _ref?: string } }; imageUrl?: string }[];
  ctaHeadline?: string;
  ctaSubtext?: string;
  contactEmail?: string;
}
...
export default function CommercialPage({ data, siteSettings }: { data?: SanityData; siteSettings?: SiteSettings }) {
  ...
  const solutions = (data?.solutions?.length ? data.solutions : FALLBACK_SOLUTIONS).map((sol, i) => ({
    ...FALLBACK_SOLUTIONS[i],
    ...sol,
    image: getSanityImageSrc((sol as any).image, 800, 480) || (sol as any).imageUrl || FALLBACK_SOLUTIONS[i]?.image || "",
  }));

  // Merge Sanity stats with fallbacks
  const statsData = data?.stats?.length ? data.stats : [
  ...
  return (
    <>
        ...
        <a href="/" style={{ display: "flex", alignItems: "center", textDecoration: "none" }}>
          {logoSrc ? (
            ...
            <img
              src={logoSrc}
              alt={companyName}
              style={{ width: logoMaxWidth, height: "auto", maxHeight: logoMaxHeight, padding: logoPadding }}
            />
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
      ...
      {/* ═══ VENDOR MODAL ═══ */}
      <VendorModal isOpen={modalOpen} onClose={() => setModalOpen(false)} email={EMAIL} />
    </>
  );
}
